...
ShipmentStatus status; bool isPaid;}
mapping(address => Shipment []) public shipments; uint256 public shipmentCount;
struct TyepShipment {
address sender;
address receiver;
uint256 pickupTime;
...
shipmentCount++;
tyepShipments.push( TyepShipment(
msg.sender, 
_receiver, 
_pickupTime,
0,
...
function startShipment (address _sender, address _receiver, uint256 _index) public { Shipment storage shipment = shipments [_sender] [_index]; TyepShipment storage tyepShipment = tyepShipments [_index];
...
tyepShipment.deliveryTime = block.timestamp; shipment.deliveryTime = block.timestamp;
...
function getShipmentsCount(address _sender) public view returns (uint256) { return shipments [_sender].length;
}
